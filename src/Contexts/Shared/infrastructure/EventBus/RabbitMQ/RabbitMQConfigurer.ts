...
export class RabbitMQConfigurer {
  ...
  async configure(params: { exchange: string; subscribers: Array<DomainEventSubscriber> }): Promise<void> {
  ...
  private getRoutingKeysFor(subscriber: DomainEventSubscriber) {
    const routingKeys = subscriber.subscribedTo().map((event) => event);
