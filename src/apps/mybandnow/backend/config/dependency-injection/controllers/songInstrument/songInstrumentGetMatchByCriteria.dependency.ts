import { ContainerBuilder, Reference } from 'node-dependency-injection';
import SongInstrumentGetMatchByCriteriaController from '../../../../controllers/songInstrument/SongInstrumentGetMatchByCriteriaController.js';

export const register = (container: ContainerBuilder) => {
  container
    .register(
      'Apps.Mybandnow.Backend.controllers.SongInstrumentGetMatchByCriteriaController',
      SongInstrumentGetMatchByCriteriaController
    )
    .addArgument(new Reference('Shared.BunyanLogger'))
    .addArgument(new Reference('Shared.CommandBus'))
    .addArgument(new Reference('Shared.QueryBus'))
    .addArgument(new Reference('Shared.Express.ApiExceptionsHttpStatusCodeMapping'));
};
