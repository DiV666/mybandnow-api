...
describe('VideoclipProcessRequester', () => {
  ...
  it.each(['PENDING', 'MIXING'])(
    ...
    async (status) => {
      ...
      const command = new RequestVideoclipCommand(ID, SONG_ID, ORIGINAL_VIDEOCLIP_URL, [
        { songInstrumentId: SONG_INSTRUMENT_ID, videoUrl: 'gs://x.mp4', instrumentName: 'Guitar', startTimeMs: 0 }
      ]);
  ...
  it('throws IncompleteSongInstrumentsException when an instrument has no video', async () => {
    ...
    const command = new RequestVideoclipCommand(ID, SONG_ID, ORIGINAL_VIDEOCLIP_URL, [
      { songInstrumentId: SONG_INSTRUMENT_ID, videoUrl: null, instrumentName: 'Guitar', startTimeMs: 0 }
    ]);
