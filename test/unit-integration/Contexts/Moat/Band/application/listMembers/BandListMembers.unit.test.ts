...
import { BandListMembersQuery } from '@Contexts/Moat/Band/application/listMembers/BandListMembersQuery.js';
import { BandListMembersResponse } from '@Contexts/Moat/Band/application/listMembers/BandListMembersResponse.js';
...
import { BandOwnerId } from '@Contexts/Moat/Band/domain/value-object/BandOwnerId.js';
...
describe('BandListMembers', () => {
  it('returns the band members for an authenticated band member', async () => {
    ...
    const band = BandMother.create({
      ...
      members: [
        new BandMember(new BandMemberId(BandMemberId.random()), new MusicianId(ownerId), new BandMemberRole('ADMIN')),
        new BandMember(new BandMemberId(BandMemberId.random()), new MusicianId(memberId), new BandMemberRole('MEMBER'))
      ]
    });
